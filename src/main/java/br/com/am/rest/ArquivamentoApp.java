...
public class ArquivamentoApp extends RestApp {
	...
	public Response arquivamento(@HeaderParam("token") String token,
		...
		try {
			if (arquivamento.getId() == null) {
				...
				Arquivamento arquivamentoBase = session.get(Arquivamento.class, arquivamento.getId());
				if (arquivamento.getVersao() < arquivamentoBase.getVersao()) {
					return Response.status(400).entity(StringEscapeUtils
							.escapeHtml4("Arquivamento foi alterado por : "
	...
	private Response processaValidacoes(Arquivamento arquivamento, String token) {
		if (Util.isNullOrEmpty(arquivamento.getCodigo())) {
			return Response.status(400)
					.entity(StringEscapeUtils.escapeHtml4("Código obrigatório."))
					.type(MediaType.APPLICATION_JSON).build();
		}
		if (arquivamento.getEmpresa() == null) {
			return Response.status(400)
					.entity(StringEscapeUtils.escapeHtml4("Empresa obrigatória."))
					.type(MediaType.APPLICATION_JSON).build();
		}
		if (Util.isNullOrEmpty(arquivamento.getDescricao())) {
			return Response.status(400)
					.entity(StringEscapeUtils.escapeHtml4("Descrição obrigatória."))
					.type(MediaType.APPLICATION_JSON).build();
		}
		if (arquivamento.getTipoArquivamento() == null) {
			return Response.status(400)
					.entity(StringEscapeUtils
							.escapeHtml4("Tipo de arquivamento obrigatório."))
					.type(MediaType.APPLICATION_JSON).build();
		}
		if (arquivamento.getDataReferencia() == null) {
			return Response.status(400)
					.entity(StringEscapeUtils
							.escapeHtml4("Data referêcia obrigatória."))
					.type(MediaType.APPLICATION_JSON).build();
		}
		if ((arquivamento.getTipoExpurgo() == null
			...
			return Response.status(400)
					.entity(StringEscapeUtils
							.escapeHtml4("Data de expurgo obrigatória."))
					.type(MediaType.APPLICATION_JSON).build();
		}
		return null;
	}
	...
	public Response apagarArquivamento(@HeaderParam("token") String token,
		...
		try {
			if (arquivamento.getId() == null) {
				return Response.status(400)
						.entity(StringEscapeUtils.escapeHtml4("Arquivamento inválido"))
						.type(MediaType.APPLICATION_JSON).build();
			} else {
				Arquivamento arquivamentoBase = session.get(Arquivamento.class, arquivamento.getId());
