...
O Logback DEVE permitir configurar níveis de log diferentes por pacote, com suporte a herança de nível. O nível global DEVE ser WARN, o pacote `br.com.am` DEVE ser INFO, e o pacote `org.hibernate.SQL` DEVE ser DEBUG (opcional, ativado por configuração).
...
O sistema DEVE redirecionar todo logging de `java.util.logging` (usado por Tomcat e BIRT) para SLF4J/Logback, garantindo que logs do Catalina (Tomcat), BIRT e demais bibliotecas que usam JUL apareçam no mesmo arquivo `logs/am-sis.log`.
...
Em ambiente de desenvolvimento (perfil não-produção), o Logback DEVE exibir logs no console com formato colorido para facilitar debug.

#### Scenario: console output in dev mode
- **WHEN** a aplicação roda sem a property `-Dprod=true`
